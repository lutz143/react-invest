...
export const signup = createAsyncThunk('auth/api/users', async ({ id, username, password, email }, thunkAPI) => {
    try {
        const res = await axios.post('http://localhost:3001/api/users', { id, username, password, email })
        return res.data
    } catch (err) {
        console.log(err)
